...
export default function Home() {
  ...
  useEffect(() => {
    const updateProgress = () => {
      ...
      if (milestone) {
        ...
        const localProgress = Math.max(0, Math.min(1, (window.innerHeight * 0.82 - bounds.top) / Math.max(1, bounds.height * 0.9)));
        setPyramidFrame(Math.min(7, Math.floor(localProgress * 8)));
      }
    };
  ...
  return <div className="site-shell"><div className="cursor-glow" aria-hidden="true" /><div className="page-progress" style={{ width: `${scrollProgress}%` }} aria-hidden="true" />
    ...
    <main id="top">
      ...
      <section className="milestone container" data-reveal><div className="milestone-art"><img src={ASSETS.hero} alt="Pixel art Alexandria coastline at dusk" /></div><div className="milestone-copy"><span className="section-index">04 / NEXT CHECKPOINT</span><h2>KEEP<br /><em>SHIPPING</em></h2><p>The next level is not a title. It’s another useful thing in the hands of a real person.</p><div className="pyramid-build" aria-label={`Pixel pyramid build stage ${pyramidFrame + 1} of 8`}><div className="pyramid-sprite" style={{ backgroundImage: `url(${ASSETS.pyramidSprites})`, backgroundPosition: `${(pyramidFrame % 4) * 33.3333}% ${Math.floor(pyramidFrame / 4) * 100}%` }} /><div className="pyramid-status"><span>BUILDING PYRAMID</span><strong>STAGE {String(pyramidFrame + 1).padStart(2, "0")} / 08</strong></div></div><a href="mailto:yousefkhamismadbouly@googlemail.com" className="button primary">START A CONVERSATION <ArrowUpRight size={16} /></a></div></section>
    </main>
