...
const ASSETS = {
  chime: "data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQAAAAA=",
};

const projects = [
  { number: "01", kind: "AI / LLM", title: "Egytronic_1.0", text: "Fine-tuned 8B-parameter language model for Egyptian Arabic, local culture, and practical inference workflows.", tags: ["Llama 3.1", "Unsloth", "GGUF"], link: "https://huggingface.co/YousefKhamis/Egytronic_1.0", cta: "View on Hugging Face" },
  { number: "02", kind: "EDTECH / AI", title: "SmartBoard AI", text: "An AI-powered classroom whiteboard that turns lesson intent into visual, interactive teaching moments.", tags: ["React", "AI Teacher", "Realtime"], link: "https://smartboard-eg.pages.dev", cta: "Open live project" },
  { number: "03", kind: "PRODUCT / WEB", title: "Azura Cafe", text: "A mobile-first restaurant experience with QR access, realtime ordering, and a warm seasonal visual system.", tags: ["Firebase", "Cloudflare", "Framer Motion"], link: "https://azura-app.pages.dev", cta: "Open live project" },
];
...
function ChatModal({ onClose }: { onClose: () => void }) {
  ...
  const [messages, setMessages] = useState<{ from: "bot" | "user"; text: string }[]>([
    { from: "bot", text: "أهلاً! أنا Egytronic portfolio agent. اسألني عن مشاريع Yousef Khamis، Founder of Egytronic & Software Developer (مطور برمجيات)، الـstack، أو Egytronic_1.0." },
  ]);
  const answer = (q: string) => {
    ...
    return "أقدر أساعدك في فهم خبرة Yousef Khamis (Founder of Egytronic & Software Developer)، Egytronic_1.0، SmartBoard AI، Azura Cafe، أو اختياراته التقنية. جرّب سؤالاً أكثر تحديداً.";
  };
  const send = () => {
  ...
  return <div className="modal-backdrop" onClick={onClose}><section className="chat-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Chat with YK portfolio agent">
    <header className="chat-head"><div><div className="eyebrow"><span className="status-dot" /> ONLINE / CV CONTEXT LOADED</div><h2>EGYTRONIC_AGENT<span className="cursor">_</span></h2></div><button className="icon-button" onClick={onClose} aria-label="Close chat"><X size={18} /></button></header>
    <div className="chat-body">{messages.map((m, i) => <div className={`message ${m.from}`} key={i}><span className="message-label">{m.from === "bot" ? "egytronic_agent" : "you"}</span><p>{m.text}</p></div>)}</div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    updateProgress();
    return () => { revealObserver.disconnect(); window.removeEventListener("scroll", updateProgress); };
  }, []);
  ...
  return <div className="site-shell"><div className="page-progress" style={{ width: `${scrollProgress}%` }} aria-hidden="true" />
    <nav className="nav container"><a href="#top" className="brand" aria-label="Yousef Khamis home"><MascotIcon className="small" /><span>EGYTRONIC<span className="muted">.DEV</span></span></a><div className={`nav-links ${menuOpen ? "open" : ""}`}><a href="#work" onClick={() => setMenuOpen(false)}>WORK</a><a href="#stack" onClick={() => setMenuOpen(false)}>STACK</a><a href="#about" onClick={() => setMenuOpen(false)}>ABOUT</a><a href="/llm.txt" download>LLM.TXT</a></div><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation"><Menu size={20} /></button><button className="chat-trigger" onClick={() => setChatOpen(true)}><MessageSquare size={15} /> ASK YK</button></nav>
    <main id="top">
      <section className="hero container" data-reveal><div className="hero-copy"><div className="eyebrow"><span className="status-dot" /> ALEXANDRIA, EGYPT / AVAILABLE FOR SELECT BUILDS</div><h1>FOUNDER OF<br /><span>EGYTRONIC</span><span className="cursor">_</span></h1><p className="hero-lede">Software Developer (مطور برمجيات) crafting full-stack products, applied AI, and practical interfaces with a distinctly Egyptian point of view.</p><div className="hero-actions"><a href="#work" className="button primary">EXPLORE THE WORK <ArrowUpRight size={16} /></a><button className="button ghost" onClick={play}><Play size={14} /> {playing ? "PLAYING..." : "PLAY SIGNAL"}</button></div><div className="hero-meta"><span><Terminal size={14} /> React / AI / SHIP</span><span>04°12′N / 31°14′E</span></div></div><div className="hero-art"><div className="art-frame"><MascotIcon /><span className="art-tag">[ MASCOT // EGYTRONIC ]</span></div><div className="scroll-note">SCROLL TO LOAD STORY <span>↓</span></div></div></section>
      ...
      <section id="work" className="section container" data-reveal><div className="section-heading"><div><span className="section-index">01 / SELECTED WORK</span><h2>THINGS I<br /><em>SHIPPED</em></h2></div><p>Real products, public deployments, and experiments that move between code, culture, and users.</p></div><div className="project-grid">{projects.map((p) => <article className="project-card" key={p.title}><div className="project-image-placeholder"><span className="project-number">{p.number}</span><span className="project-tag-pill">{p.kind}</span><span className="live-pill"><span className="status-dot" /> LIVE</span></div><div className="project-info"><div className="project-title-row"><h3>{p.title}</h3></div><p>{p.text}</p><div className="tags">{p.tags.map((t) => <span key={t}>{t}</span>)}</div><AppLink href={p.link} className="text-link">{p.cta} <ArrowUpRight size={15} /></AppLink></div></article>)}</div></section>
